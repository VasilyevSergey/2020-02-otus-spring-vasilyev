...
public class DatabaseChangelog {
    ...
    public void addBooks(MongoTemplate template) {
        val firstBook = new Book("Ruslan and Lyudmila", pushkin.getId(), poem.getId());
        val secondBook = new Book("Lord of the Rings", tolkien.getId(), fantasy.getId());
        val thirdBook = new Book("thirdBook", tolkien.getId(), poem.getId());

        ruslanAndLudmila = template.save(firstBook);
        lordOfTheRings = template.save(secondBook);
        template.save(thirdBook);
    }
