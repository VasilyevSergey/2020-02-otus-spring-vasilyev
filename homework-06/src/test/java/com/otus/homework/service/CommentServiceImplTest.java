...
import com.otus.homework.dao.CommentDaoJpa;
...
class CommentServiceImplTest {
    ...
    void shouldReturnExpectedCommentById() {
        given(dao.getById(TEST_COMMENT_ID))
    ...
    void shouldAddComment() {
        ...
        verify(dao, times(1)).add(comment);
    }
    ...
    void shouldThrowExceptionIfCantAddComment() {
        doThrow(RecoverableDataAccessException.class).when(dao).add(isA(Comment.class));
    ...
    void shouldDeleteCommentById() {
        given(dao.deleteById(TEST_COMMENT_ID))
                .willReturn(1);
    ...
    void shouldUpdateComment() {
        given(dao.getById(UPDATED_TEST_COMMENT.getId()))
                .willReturn(Optional.of(UPDATED_TEST_COMMENT));

        given(dao.update(UPDATED_TEST_COMMENT))
                .willReturn(1);

        service.updateById(TEST_COMMENT_ID, UPDATED_COMMENT_TEXT);
        verify(dao, times(1)).update(UPDATED_TEST_COMMENT);
    }
    ...
    void shouldThrowExceptionWhileUpdateIfCommentNotFound() {
        doThrow(RecoverableDataAccessException.class).when(dao).update(UPDATED_TEST_COMMENT);
