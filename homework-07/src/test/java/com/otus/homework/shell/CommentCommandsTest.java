...
class CommentCommandsTest {
    ...
    private static final String PATTERN_SUCCEED_DELETE_COMMENT = "Комментарий с id '%d' удален";
    ...
    void shouldAddComment() {
        ...
        String result = (String) shell.evaluate(() -> String.format(
                "%s %s %d %s", COMMAND_ADD_COMMENT, EXPECTED_COMMENT_TEXT, BOOK_ID, TEST_COMMENTATOR));
        ...
        result = (String) shell.evaluate(() -> String.format(
                "%s %s %d %s", COMMAND_ADD_COMMENT_SHORT, EXPECTED_COMMENT_TEXT, BOOK_ID, TEST_COMMENTATOR));
        assertThat(result).isEqualTo(String.format(PATTERN_SUCCEED_ADD_COMMENT, EXPECTED_COMMENT_TEXT));
    ...
    void shouldGetCommentById() {
        ...
        String result = (String) shell.evaluate(() -> String.format("%s %d", COMMAND_GET_COMMENT, EXPECTED_COMMENT_ID));
        assertThat(result).isEqualTo(EXPECTED_COMMENT.toString());
        verify(service, times(1)).getById(EXPECTED_COMMENT_ID);

        result = (String) shell.evaluate(() -> String.format("%s %d", COMMAND_GET_COMMENT_SHORT, EXPECTED_COMMENT_ID));
        assertThat(result).isEqualTo(EXPECTED_COMMENT.toString());
    ...
    void shouldDeleteCommentById() {
        ...
        String result = (String) shell.evaluate(() -> String.format("%s %d", COMMAND_DELETE_COMMENT, EXPECTED_COMMENT_ID));
        ...
        result = (String) shell.evaluate(() -> String.format("%s %d", COMMAND_DELETE_COMMENT_SHORT, EXPECTED_COMMENT_ID));
        assertThat(result).isEqualTo(String.format(PATTERN_SUCCEED_DELETE_COMMENT, EXPECTED_COMMENT_ID));
